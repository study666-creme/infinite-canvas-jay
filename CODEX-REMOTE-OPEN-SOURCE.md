...
`/codex-remote` 和当前本地 bridge 中的 Codex 控制能力，本质上是一个自托管的 **Codex Remote Console**：手机网页连接用户自己电脑上的 bridge，再由 bridge 调用本机 Codex app-server 继续项目任务。旧入口 `/mobile-agent` 只作为兼容路由保留。
...
- 不要只依赖前端限额保护公共入口；公开服务必须在服务端校验账号和用量。
- 不要转售、出租或共享账号访问能力。
