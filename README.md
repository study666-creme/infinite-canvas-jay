...
- **画布 Agent**：网页 Agent 或本机 Agent 读取真实画布，通过通用工具创建节点、连接流程、调用生成并按需维护项目状态，不强制套用固定创作流程。
- **3D 导演台**：把文字分镜转换为角色、道具、机位和 FOV，支持手动微调、镜头切换与批量生成 `1280x720` 预演截图。
- **可选本地资料工具**：可在私有环境摄取有权使用的书摘、笔记、字幕和案例，并进行提炼与二次审核；生成物默认被 Git 忽略，公开构建不携带，本机 Agent 可在本地文件存在时按任务召回已确认知识。
...
- 公开仓库不包含维护者的蒸馏知识正文、来源材料、审核报告或个人 Agent 预设；相关路径已加入 `.gitignore`。
