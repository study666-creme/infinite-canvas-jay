...
async function writeGeneratedCases(outputFile, cases, { mode }) {
    const meta = {
        ...
        activeCaseCount: cases.filter((item) => item.status === "verified" || item.status === "auto_verified").length,
        candidateCaseCount: cases.filter((item) => item.status === "candidate").length,
        mode,
    };
