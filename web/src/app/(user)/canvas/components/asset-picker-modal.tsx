    ...
    return (
        <Modal title="选择素材" open={open} onCancel={onClose} footer={null} width={860} centered destroyOnHidden styles={{ body: { padding: "0 24px 24px", minHeight: 480 } }}>
            <Tabs
                ...
                onChange={setTab}
                items={[
                    { key: "local", label: "我的素材", children: <MyAssetsTab onInsert={onInsert} /> },
                    { key: "prompt-hub", label: "Prompt Hub 卡片库", children: <PromptHubCardsTab onInsert={onInsert} /> },
                ]}
            />
        </Modal>
    );
...
function MyAssetsTab({ onInsert }: { onInsert: (payload: InsertAssetPayload) => void }) {
    ...
    const handleInsert = (asset: Asset) => {
        if (asset.kind === "text") {
            ...
            onInsert(asset.kind === "video" ? { kind: "video", url: asset.data.url, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height } : { kind: "image", dataUrl: asset.data.dataUrl, storageKey: asset.data.storageKey, title: asset.title, prompt: asset.metadata?.prompt as string | undefined });
        }
    };
