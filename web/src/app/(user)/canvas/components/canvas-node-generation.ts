...
import { extractMentionedReferenceLabels } from "../utils/canvas-video-references";
...
export function buildNodeGenerationContext(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[], prompt: string): NodeGenerationContext {
    ...
    const upstreamText = inputs
        ...
        .filter(Boolean)
        .join("\n\n");
    const selectedInputs = selectInputsByPromptLabels(nodeId, nodes, connections, prompt, inputs);
...
function selectInputsByPromptLabels(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[], prompt: string, inputs: NodeGenerationInput[]) {
    const mentionedLabels = extractMentionedReferenceLabels(prompt);
    if (!mentionedLabels.size) return inputs;
    const sourceNode = nodes.find((node) => node.id === nodeId);
    if (!sourceNode) return inputs;
    const labelByNodeId = new Map(buildNodeMentionReferences(sourceNode, nodes, connections).map((reference) => [reference.nodeId, reference.label]));
    return inputs.filter((input) => {
        const label = labelByNodeId.get(input.nodeId);
        return Boolean(label && mentionedLabels.has(label));
    });
}
