...
- 输出任何剧本、分镜、视频提示词前，内部做一次「故事是否成立、镜头是否可拍、AI 是否可生成、短视频是否能留人」的二次审核。
...
- 当前项目可披露的创作知识状态是：已内置一套「高质量源二次审核版」创作知识核心，覆盖故事、人物、剧本、导演、表演、分镜、摄影、色彩、剪辑、声音、AI 视频生成和上线质检；其来源范围包括用户指定 UP 主「天命斩水」B站「写作教学」合集（season_id 5909257，共 60 条，主题覆盖结构、期待感、情绪、爽点、知识、心理、悬念等），以及高可信故事/剧作、导演/表演、视听/摄影/分镜/色彩、剪辑/声音和 AI 视频官方文档方向。
...
2. 创作知识核心：默认隐性应用已内置的高质量创作知识，覆盖网文/短剧叙事、结构人物、剧本场景、导演视听、视觉剪辑声音、AI 视频生成、生产复盘；不要创建空知识库节点，除非用户要求展示。
...
export function applyShortDramaAgentMode(prompt: string, mode: CanvasAgentCreativeMode) {
    ...
    const context = [
        ...
        shouldUseCreativeKnowledge && mode !== "short_drama" ? CREATIVE_KNOWLEDGE_CORE_CONTEXT : "",
...
export function shouldDiscloseCanvasAgentKnowledge(prompt: string, mode: CanvasAgentCreativeMode) {
    ...
    return /学习库|知识库|方法卡|学了|学过|吸收|填进去|补进去|内置.*(?:书|视频|内容)|书籍|视频|方法论|创作知识|高质量知识|天命斩水|短剧创作总监|导演|拉片|视听|网感|审美|爆款|平台传播/.test(prompt);
}

export function shouldUseCanvasCreativeKnowledge(prompt: string, mode: CanvasAgentCreativeMode) {
    if (mode === "short_drama") return true;
    return /创作|短剧|故事|剧情|剧本|编剧|人物|角色|人设|IP|爽点|悬念|反转|情绪|节奏|分镜|镜头|导演|表演|调度|摄影|构图|色彩|光线|剪辑|声音|音效|配乐|拉片|视频生成|AI\s*视频|图生视频|文生视频|提示词|视觉总纲|封面|标题|预告|连续性|一致性|网感|审美|爆款|传播|留存|完播|互动|评论区|抖音|快手|小红书|B站|TikTok|Reels|Shorts/.test(prompt);
}
