...
export function ImageSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", maxCount = 15, quickCount = 10, variant = "default" }: ImageSettingsPanelProps) {
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            ...
            >
                {showTitle && !jimeng ? <div className="text-lg font-semibold">图像设置</div> : null}
                {jimeng ? (
                    <>
                        <div className="space-y-3">
                            <JimengSectionTitle color={theme.node.muted}>选择比例</JimengSectionTitle>
                            <JimengRatioGrid
                                options={jimengRatioOptions}
                ...
                <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-4 gap-2.5">
                        {Array.from({ length: variant === "jimeng" ? 4 : quickCount }, (_, index) => index + 1).map((value) => (
