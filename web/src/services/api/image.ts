...
import { isQianfanTextConfig } from "@/lib/qianfan-text";
import { requestQianfanText, requestQianfanToolResponse } from "@/services/api/chat-completions";
...
export async function requestToolResponse(config: AiConfig, messages: ResponseInputMessage[], tools: ResponseFunctionTool[], toolChoice: ToolChoice = "auto", onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    ...
    try {
        ...
        if (isQianfanTextConfig(requestConfig)) {
            return await requestQianfanToolResponse(requestConfig, withSystemMessage(requestConfig, messages), tools, toolChoice, onDelta, options);
        }
